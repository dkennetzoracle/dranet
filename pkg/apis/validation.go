...
func ValidateRDMAOnlyConfig(raw *runtime.RawExtension) []error {
	...
	if config.Interface.Name != "" || config.Interface.Type != "" ||
		...
		config.Interface.AcceptRA != nil || config.Interface.IPVlan != nil {
...
func validateSubinterfaceOnlyConfig(cfg *InterfaceConfig, fieldPath string) (allErrors []error) {
	if cfg.MTU != nil || cfg.HardwareAddr != nil ||
		...
		allErrors = append(allErrors, fmt.Errorf("%s: mtu, hardwareAddr, gso/gro sizes, arpIgnore, arpAnnounce, acceptRA and dhcp are not yet supported for subinterface types (type: %s)", fieldPath, cfg.Type))
	}
	return allErrors
}
